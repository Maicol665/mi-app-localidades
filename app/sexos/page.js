...
export default function SexosPage() {
  ...
  const iniciarEdicion = (item) => {
    setEditId(item.id_sexo);
    setEditVal(item.sexo);
    setMsg('');
  };

  const guardar = async (e) => {
    e.preventDefault();
    const res = await fetch(`/api/sexos/${editId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sexo: editVal }),
    });
  ...
  return (
    <div className={styles.container}>
      <h1 className={styles.title}>Actualizar Sexos</h1>
      ...
      <table className={styles.table}>
        <thead><tr><th>ID</th><th>Sexo</th><th>Acción</th></tr></thead>
        <tbody>
          {data.map(item => (
            <tr key={item.id_sexo}>
              <td>{item.id_sexo}</td>
